...
class Interpreter
{
    ...
    {
        if (! is_string($apiKey) || $apiKey === '' || empty($this->config['API_KEYS'])) {
            return false;
        }

        $validKeys = array_filter(array_map('trim', explode(',', (string) $this->config['API_KEYS'])));
    ...
    public function run(string $input): void
    {
        ...
        // a valid API key (trusted server-side submissions) bypasses CSRF validation only
        $hasValidApiKey = $this->validateApiKey($payload['apiKey'] ?? null);
        ...
        if (
            ...
            || (
                ...
                && (
                    ...
                    || $payloadCsrfToken !== ($_SESSION['csrf_token'] ?? null)
                )
            )
